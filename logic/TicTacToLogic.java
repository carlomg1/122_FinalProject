...
public class TicTacToLogic implements GameLogic {
	...
	public boolean checkValidMove (buttonListener button){
		System.out.println("HERE"+button.checkDiagonalWin());
		if (button.checkDiagonalWin()==true) {
		return true;
	}
		return false;
	}
	 ...
	 private class buttonListener extends JButton implements ActionListener
	    {
		 ...
		 public buttonListener() {
	        ...
	        {
	            if (buttons[row].getText().equals(buttons[column].getText()) && !buttons[row].getText().equals(""))
	                return true;
	            else
	                return false;
	        }
	...
	public void initializeBoard() {
		...
		for(int i=0; i<=numberButtons-1; i++){ //placing the button onto the board
			...
			checkValidMove(buttons[i]);
			System.out.println("PASSES HERE"+checkValidMove(buttons[i]));
			panel.add(buttons[i]);	
